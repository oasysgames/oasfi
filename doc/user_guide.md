...
* The default is commission-reward.csv (validator reward is commission-reward)
...
* This is a script designed to generate records of money generated by staker rewards for a specific address.
* You need to specify the Staker Owner key as the address.
* To retrieve the price of the OAS token, you need to obtain an API key from CoinGecko and execute the following command. (Please note that you should contract with CoinGecko and obtain the API key yourself.)
...
* Specify the output file destination.
* The default is staker-reward.csv
