...
# File paths
NEW_YORK_FILE="./output_csv/staker-reward_NewYork.csv"
SINGAPORE_FILE="./output_csv/staker-reward_Singapore.csv"
...
convert_to_unix() {
  ...
  # Parse the formatted timestamp to Unix time (compatible with macOS)
  date -j -f "%Y-%m-%d %H:%M:%S" "$formatted_timestamp" "+%s" 2>/dev/null
}
